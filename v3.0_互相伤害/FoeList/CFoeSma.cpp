...
void CFoeSma::ShowFoe1()
{
	::putimage(m_x, m_y, FOESMA_WIDTH * 2, FOESMA_HEIGHT * 2,
		&m_imgFoe, FOESMA_WIDTH * 2, (FOESMASHOWID_INIT - m_showId) * FOESMA_HEIGHT * 2, SRCPAINT);
	::putimage(m_x, m_y, FOESMA_WIDTH * 2, FOESMA_HEIGHT * 2,
		&m_imgFoe, 0, (FOESMASHOWID_INIT - m_showId) * FOESMA_HEIGHT * 2, SRCAND);
}
...
bool CFoeSma::IssHitGun(CGunner* Gun)
{
	int x = Gun->m_x;
	int y = Gun->m_y;
	/*if (x >= m_x && x <= m_x + FOESMA_WIDTH && y >= m_y && y <= m_y + FOESMA_HEIGHT)
	{
		return true;
	}*/
	if (Gun->type == 0)
