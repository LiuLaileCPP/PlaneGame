...
void CFoeMid::InitFoe(int size)
{
	...
	{
		m_x = (rd()) % (BACK_WIDTH - FOEMID_WIDTH * 2 + 1);
		m_y = -FOEMID_HEIGHT * 2;
		::loadimage(&m_imgFoe, L"jpg", L". / res / foeplaneMID.jpg", FOEMID_WIDTH * 2, FOEMID_HEIGHT * 2, true);
		m_blood = FOEMID_BLOOD;
		m_showId = FOEMIDSHOWID_INIT;
	}
}
...
void CFoeMid::ShowFoe1()
{
	::putimage(m_x, m_y, FOEMID_WIDTH * 2, FOEMID_HEIGHT * 2,
		&m_imgFoe, FOEMID_WIDTH * 2, (FOEMIDSHOWID_INIT - m_showId) * FOEMID_HEIGHT * 2, SRCPAINT);
	::putimage(m_x, m_y, FOEMID_WIDTH * 2, FOEMID_HEIGHT * 2,
		&m_imgFoe, 0, (FOEMIDSHOWID_INIT - m_showId) * FOEMID_HEIGHT * 2, SRCAND);
}
...
bool CFoeMid::IssHitGun(CGunner* Gun)
{
	int x = Gun->m_x;
	int y = Gun->m_y;
	/*if (x >= m_x && x <= m_x + FOEMID_WIDTH && y >= m_y && y <= m_y + FOEMID_HEIGHT)
	{
		return true;
	}*/
	if (Gun->type == 0)
